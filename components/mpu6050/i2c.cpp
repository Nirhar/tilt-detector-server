...
I2C::I2C(gpio_num_t scl, gpio_num_t sda, i2c_port_t port) {
    ...
    int ret = i2c_driver_install(port, conf.mode, 0, 0, 0);
    if(ret == ESP_OK){
    	printf("Initialization of i2c ok\n");
    	printf("GPIO values = %d  %d\n",GPIO_NUM_22,GPIO_NUM_21);
    }
}
...
bool I2C::slave_read(uint8_t slave_addr, uint8_t data, uint8_t *buf, uint32_t len) {
    ...
    i2c_ack_type_t ack = len==1 ? I2C_MASTER_NACK : I2C_MASTER_ACK;
    while(len) {
        i2c_master_read_byte(cmd, buf, ack);
        buf++;
        len--;
    }
    i2c_master_stop(cmd);
